...
def classify_msb_leg(atr_move: float, range_ratio: float, body_quality: float, position_score: float) -> str:
    ...
    if atr_move < c["atr_short_max"] and range_ratio < c["range_short_max"] and body_quality < c["body_short_max"] and position_score < c["position_short_max"]:
        return "SHORT"
...
def build_msb_ob_context(klines_1h: list[dict[str, Any]], liquidity_ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    mid = (zone_low + zone_high) * zcfg["continuation_mid_ratio"] + (zone_low + zone_high) * (1 - zcfg["continuation_mid_ratio"]) / 2
    pad = atr * zcfg["merge_width_atr_mult"]

    return {
        "direction": direction,
        "leg_type": leg_type,
        "quality": min(100, int((atr_move * 0.35 + range_ratio * 0.30 + body_quality * 0.2 + min(position_score, 1.0) * 0.15) * 100)),
        "structure_zone": (round(zone_low, 2), round(zone_high, 2)),
        "order_block_zone": (round(min(float(prev["low"]), zone_low), 2), round(max(float(prev["high"]), zone_high), 2)),
        "mid_observe_zone": (round(mid - pad, 2), round(mid + pad, 2)),
        "metrics": {"atr_move": atr_move, "range_ratio": range_ratio, "body_quality": body_quality, "position_score": position_score},
    }

...
from engine.trend_snapshot import make_snapshot_key
...
class SMCTScanner:
    ...
    def _htf_context(self, klines_4h: list[dict], direction_1h: str) -> dict[str, Any]:
        ...
        if relation == "counter" and abs(float(klines_4h[-1].get("macd", 0))) > 50:
    ...
    def run_once(self) -> dict[str, Any]:
        try:
            ...
            htf = self._htf_context(klines_4h, "long" if msb["direction"] == "bull" else "short")
            decision = decide_trend_segment(self.symbol, "1h", htf, liq, msb, matrix, aux)

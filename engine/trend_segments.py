...
def decide_trend_segment(
    ...
    sweep_match = (
        (direction == "long" and liq["sweep_type"] == "sellside" and liq["reclaim_or_reject"] == "reclaim")
        or (direction == "short" and liq["sweep_type"] == "buyside" and liq["reclaim_or_reject"] == "reject")
    )
    ...
    if direction != "neutral":
        if has_sweep and msb["leg_type"] in {"MID", "LONG", "EXTENDED"}:
    ...
    elif has_sweep and msb["leg_type"] == "SHORT":
    ...
    if alert_type.endswith("CONTINUATION"):
        ...
        zone = msb["mid_observe_zone"]
    elif has_sweep:
        zone_source = "structure_shift_zone"
        zone = msb["structure_zone"]
    else:
        zone_source = "context_zone"
        zone = msb["order_block_zone"]
    ...
    return {
        ...
        "htf_context": htf_ctx["text"],
        "htf_relation": htf_ctx["relation"],
        "sweep_type": liq["sweep_type"],
        "sweep_level": liq["sweep_level"],
        "reclaim_or_reject": liq["reclaim_or_reject"],
        "msb_direction": msb["direction"],
        "msb_quality": msb["quality"],
        "msb_atr_ratio": msb["metrics"]["atr_move"],
        "msb_range_ratio": msb["metrics"]["range_ratio"],
        "msb_body_quality": msb["metrics"]["body_quality"],
        ...
        "state_version": msb["leg_type"],
        "momentum_desc": aux["momentum_desc"],
        "temperature_desc": aux["temperature_desc"],
    }

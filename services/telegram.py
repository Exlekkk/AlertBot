...
def _format_x_message(
    ...
    type_text = abnormal_type or ("放量上破 / 可能空头回补" if signal == "X_BREAKOUT_LONG" else "放量下破 / 可能多头踩踏")
    level_label = "关键上破位" if signal == "X_BREAKOUT_LONG" else "关键下破位"
    level_text = f"{float(trigger_level):.2f}" if trigger_level is not None else "观察15m近端结构位"
    zone_label = "回踩观察区" if signal == "X_BREAKOUT_LONG" else "反抽观察区"
    return (
        f"{prefix} 异动预警｜X类\n"
        f"操作建议：{action_text}\n"
        f"标的：{symbol}\n"
        f"异动类型：{type_text}\n"
        f"{level_label}：{level_text}\n"

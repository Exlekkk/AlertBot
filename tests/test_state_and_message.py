...
class TrendSegmentTests(unittest.TestCase):
    def _base(self, leg, sweep_level=100):
        liq={"sweep_type":"sellside","reclaim_or_reject":"reclaim","sweep_level":sweep_level,"recent_sweep_valid":True,"bars_since_sweep":1,"prev_low":95,"prev_high":120}
    ...
    def test_recent_sweep_uses_sweep_level_as_invalid(self):
        d=self._base("MID", sweep_level=101.5)
        self.assertEqual(d["invalid_level"], 101.5)
...
class MessageTests(unittest.TestCase):
    ...
    def test_banned_terms_no_leak(self):
        from engine.trend_messages import format_trend_message
        msg = format_trend_message({"direction":"long","zone":(100,110),"htf_context":"4H 偏多","momentum_desc":"动能 偏强","temperature_desc":"热度 中性","invalid_level":99})
        for t in ["liquidity","MSB","OB","SMC","ICT","RAR","TAI","流动性","订单块"]:
